// CXFont Decrypt Engine (Typr.js + MD5 + decrypt)
...
// Font decryption function
// Returns the char_map as JSON: {encrypted_code: original_code, ...}
function __cxFontDecrypt(fontBase64, ttfTable) {
...
// Decrypt and replace characters in DOM
function __cxFontDecryptAndReplace(doc, fontBase64, ttfTable) {
    var charMap = __cxFontDecrypt(fontBase64, ttfTable);
    var count = Object.keys(charMap).length;
    
    doc.querySelectorAll('.font-cxsecret').forEach(function(el) {
    ...
    return {count: count, charMap: charMap};
}
